...
***********************************************************************************************************************************
# *Count me in? How Considering Foreign Residents in the Population Statistics Affects Federal Constituency Boundaries*

Programme: Master of Data Science for Public Policy 2024
Student: Carlo Greß
Supervisor: Prof. Asya Magazinnik

Link to the GitHub repository (all required files are located there): https://github.com/carlo-gress/ma_thesis
...
btw_21 <- btw_21 |>
  left_join(wahlkreise_germany |> dplyr::select(WKR_NR, geometry), by = c("DistrictNo" = "WKR_NR"))
...
btw21_long <- btw21_df |>
  dplyr::select(deviation, deviation_non_residents) |>
...
table_latex <- btw_21 %>%
  ...
  dplyr::select(Country, MeanForeignersPercentage) %>%
...
# Save the plot
ggsave("final_figures/figure A1/deviation_hist.png", width = 12, height = 8, dpi = 300)
...
```{r}
png(filename = "final_figures/figure 6/simulations_deviations_15.png", width = 12, height = 8, units = "in", res = 300)
